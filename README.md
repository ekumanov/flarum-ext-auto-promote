...
- they are not an administrator or moderator
- their account is at least *N* hours old (default 0, i.e. no age requirement)
...
The count is re-evaluated when a user posts, and also when one of their posts is
**approved later**. That second trigger matters: without it, a member whose
qualifying post is approved after the fact stays unpromoted until they happen to
post again.
...
- **Minimum account age (hours)** — default 0 (off).
